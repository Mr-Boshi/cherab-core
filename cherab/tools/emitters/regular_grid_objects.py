...
from .regular_grid_emitters import CylindricalRegularLineEmitter, CartesianRegularLineEmitter
...
class RegularGridObject:
    ...
    def max_wavelength(self):
        return self._primitive.material.max_wavelength
    ...
    def spectral_line_bin(self):
        return self._primitive.material.spectral_line_bin

    def set_wavelength(self, min_wavelength, max_wavelength, bins):
        self._primitive.material.set_wavelength(min_wavelength, max_wavelength, bins)
...
class RegularGridCylinder(RegularGridObject):
    ...
    def __init__(self, line_emission, radius_outer, height, radius_inner=0, period=360., step=None,
                 max_wavelength_step=1., parent=None, transform=None):
        if 360. % period > 1.e-3:
        ...
        dr = (radius_outer - radius_inner) / shape[0]
        dz = height / shape[2]
        dphi = period / shape[1]
        ...
        material = CylindricalRegularLineEmitter(line_emission, grid_steps, max_wavelength_step=max_wavelength_step,
...
class RegularGridBox(RegularGridObject):
    ...
    def __init__(self, line_emission, xmax, ymax, zmax, step=None, max_wavelength_step=1., parent=None, transform=None):
        if not line_emission:
            raise ValueError("Argument 'line_emission' is an empty dictionary.")
        shape = next(iter(line_emission.values())).shape
        dx = xmax / shape[0]
        ...
        material = CartesianRegularLineEmitter(line_emission, grid_steps, max_wavelength_step=max_wavelength_step,
